...
The single root layout. No vue-router — tab state is a `ref<'process' | 'settings'>`. Both views are always mounted (`v-show`), so all components keep state when switching tabs. A fixed bottom navbar (72px) with a pill-shaped 2-tab segmented control is the primary navigation. Content container: max-width 1440px, `px-5 md:px-6 py-6 pb-24` (pb-24 clears the fixed navbar).
...
### Bottom Navbar (in `src/styles.css`)

- `.rf-navbar`: fixed bottom, 72px, white 92% + backdrop-blur(10px), top border `--rf-border`, z-50
- `.rf-navbar__inner`: max-width 420px, 44px tall, 2-col grid, pill `border-radius:9999px`, bg `--rf-surface-low`, 4px padding
- `.rf-nav-item`: 14px/600, pill radius, muted text; active state = white bg + card shadow + dark text
